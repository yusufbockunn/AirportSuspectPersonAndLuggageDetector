...
main.py
-------
Tek Pencere Mimarisi — v9  (Multi-Condition Intersection Filtering)
...
Düzeltmeler (v9):
  • on_command içindeki duplicate reset+set_command blokları kaldırıldı
  • last_prompt kontrolü kaldırıldı (farklı prompt geçişlerini engelliyordu)
  • set_command öncesi clear_command thread-safe şekilde çağrılıyor
  • _active_command için threading.Lock eklendi (race condition önlendi)
  • Worker thread'de mode, Lock ile korunan snapshot üzerinden okunuyor
"""
...
from detector       import detect_and_track, reset_counter as reset_detector
    ...
    def _worker_loop(self):
        while self._running:
            try:
                ...
                # ── Aktif komutu thread-safe snapshot al ──────────────
                with self._cmd_lock:
                ...
                # time.sleep kaldırıldı: YOLO inference zaten doğal
                # bir gecikme oluşturuyor. Ek sleep = gereksiz yavaşlama.
...
def main():
    ...
    def on_command(prompt: str):
        """
        Kullanıcıdan gelen her prompt'u işle.
        - Önce mevcut komutu temizle (clear_command)
        - Sonra yeni komutu ayrıştır ve uygula
        - last_prompt karşılaştırması KALDIRILDI: art arda aynı prompt da çalışır
        """
        prompt = prompt.strip()
        if not prompt:
            return
        ...
        # Yeni komut geldiğinde önce eskiyi temizle, sonra yenisini aktifel
        loop.clear_command()
    ...
    app.set_filter_change_callback(loop.clear_command)
